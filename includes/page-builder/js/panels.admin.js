...
jQuery( function ( $ ) {
			...
			buttons: {
				'Edit': function(){
					...
					if( $(this).attr('data-class') && $(this).attr('data-class') != newSectionClass ){
						...
						section = $('.section-container').eq( Number(sectionIndex) );
						...
						section.children('input[name$="[class]"]').val(newSectionClass);
						section.children('.section').attr('data-class',newSectionClass);
					}
					...
					if( $(this).attr('data-columns') && $(this).attr('data-columns') != newColumnsNb ){
						...
						for(var i = 0; i < oldContainerData.length; i++){
							for(var j = 0; j < oldContainerData[i]['widgets'].length; j++){
								...
								window.panels.addPanel(panel, newGrid.find('.panels-container').eq(realI), false);
							}
	...
	$( '#panels-dialog').show()
		.dialog( {
			...
			minWidth:    960,
			maxHeight:   Math.round($(window).height() * 0.9),
			open :       function () {
				$(this).data('overlay', overlay).closest('.ui-dialog').before(overlay);
			},
			close:       function () {
	...
	$('#panels-dialog .panel-type').click(function(){
		...
		panels.addPanel(panel, null, true);
