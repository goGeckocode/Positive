 ...
 */
function positive_panels_content_save_pre($content){
	global $post;

	if ( !positive_panels_setting('copy-content') ) return $content;
	...
	if ( empty( $_POST['section'] ) || empty( $_POST['grids'] ) || empty( $_POST['grid_cells'] ) || empty( $_POST['widgets'] ) || empty( $_POST['panel_order'] ) ) return $content;
	...
	$data['action'] = 'positive_panels_get_post_content';
	...
	$request = wp_remote_post( admin_url('admin-ajax.php?action=positive_panels_get_post_content'), array(
		'method' => 'POST',
		'timeout' => 5,
		'redirection' => 0,
		'body' => $data
...
add_filter('content_save_pre', 'positive_panels_content_save_pre');

/**
 * Ajax handler to get the HTML representation of the request.
 */
function positive_panels_content_save_pre_get(){
	...
	if( !empty($panels_data['widgets']) ) {
		...
		$content = positive_panels_render( $_POST['post_id'], $panels_data );
...
add_action('wp_ajax_nopriv_positive_panels_get_post_content', 'positive_panels_content_save_pre_get');
 ...
 * @return array
 */
function positive_panels_get_panels_data_from_post($form_post){
	$panels_data = array();
	...
	$panels_data['section'] = array_values( stripslashes_deep( isset( $form_post['section'] ) ? $form_post['section'] : array() ) );
	...
	return apply_filters('positive_panels_panels_data_from_post', $panels_data);
}