...
function black_studio_tinymce_admin_init() {
	global $pagenow;
	$load_editor = false;
	if ( $pagenow == "widgets.php"|| $pagenow == "post.php" ) {
		$load_editor = true;
	}
...
function black_studio_tinymce_scripts() {
	...
	if ( get_bloginfo( 'version' ) >= "3.3" ) {
		...
		wp_enqueue_script( 'black-studio-tinymce-widget', get_stylesheet_directory_uri().'/includes/black-studio-tinymce-widget/black-studio-tinymce-widget.js', array( 'jquery' ), $black_studio_tinymce_widget_version );
	}
	else {
		wp_enqueue_script( 'black-studio-tinymce-widget-legacy', get_stylesheet_directory_uri().'/includes/black-studio-tinymce-widget/black-studio-tinymce-widget-legacy.js', array( 'jquery' ), $black_studio_tinymce_widget_version );
	}
}
...
function black_studio_tinymce_styles() {
    ...
    wp_enqueue_style( 'black-studio-tinymce-widget', get_stylesheet_directory_uri().'/includes/black-studio-tinymce-widget/black-studio-tinymce-widget.css', array(), $black_studio_tinymce_widget_version );
}
...
function black_studio_tinymce_upload_iframe_src ( $upload_iframe_src ) {
	global $pagenow;
	if ( $pagenow == "widgets.php" || $pagenow == "post.php" || ( $pagenow == "admin-ajax.php" && isset ( $_POST['id_base'] ) && $_POST['id_base'] == "black-studio-tinymce" ) ) {
...
function black_studio_tinymce_editor_accessibility_mode($editor) {
	global $pagenow;
	if ( ($pagenow == "widgets.php" && isset( $_GET['editwidget'] ) && strpos( $_GET['editwidget'], 'black-studio-tinymce' ) === 0) || $pagenow == "post.php") {
		$editor = 'html';
	}
	return $editor;
}